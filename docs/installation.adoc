...
Once all services have the desired amount on replicas, Xillio API should be up and running. You can now access the Xillio API through `http://127.0.0.1/v2` and `http://127.0.0.1/oauth`. The documentation (if configured) is hosted on `http://127.0.0.1/docs`.

To see Traefik's proxy dashboard you could browse to `http://127.0.0.1:8080`.

In order to verify that the Xillio API is locally accessible you could perform a GET request to `http://127.0.0.1/v2/system/ping`.
