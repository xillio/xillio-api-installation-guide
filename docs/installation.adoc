...
Furthermore, we need to add the RabbitMQ credentials to the Xillio Engine by adding them to `engine.environment`:
...
// TODO: See whether we want to do this with a table as well as done in the minimal configuration example.

The value in `rabbit.environment.RABBITMQ_DEFAULT_USER` must be the same as `scriptagent.environment.AMQP_USER` and
`engine.environment.spring.rabbitmq.username`. The value in `rabbit.environment.RABBITMQ_DEFAULT_PASS` must be the same
as `scriptagent.environment.AMQP_PASS` and `engine.environment.spring.rabbitmq.password`.

Increase `scriptagent.deploy.replicas` in order to allow more load on this service. *Must be at least 1* for this
feature to work.
...
// TODO: I am still not sure why we need engine.host. I see it is only used for Sentry and Influx..
// TODO: Explain fields
